...
import com.example.DATN2025.dto.Bill.*;
...
public class BillController {
    ...
    /**
     * Phương thức xử lý yêu cầu In hóa đơn.
     * Trả về view "admin/print-bill" (cần phải tạo file này).
     *
     * @param maHoaDon ID của hóa đơn cần in.
     * @param model Đối tượng Model để truyền dữ liệu đến Thymeleaf.
     * @return Tên view in hóa đơn.
     */
    @GetMapping("/bill-print/{maHoaDon}")
    public String printBill(@PathVariable("maHoaDon") Long maHoaDon, Model model) {
        System.out.println("--- START PRINT BILL: " + maHoaDon); // ⬅️ Dòng 1

        Bill bill = billService.findById(maHoaDon).orElse(null);

        if (bill == null) {
            System.out.println("--- FAIL: Bill not found."); // ⬅️ Dòng 2
            return "redirect:/admin/bill-list";
        }
        System.out.println("--- SUCCESS: Bill code: " + bill.getCode()); // ⬅️ Dòng 3

        List<BillDetailProduct> billDetailProducts = billService.getBillDetailProduct(maHoaDon);

        // Kiểm tra dữ liệu chi tiết sản phẩm
        if (billDetailProducts == null || billDetailProducts.isEmpty()) {
            System.out.println("--- WARNING: Bill details list is empty or null."); // ⬅️ Dòng 4
        }

        model.addAttribute("bill", bill);
        model.addAttribute("billDetailProducts", billDetailProducts);

        System.out.println("--- END: Returning view admin/print-bill."); // ⬅️ Dòng 5

        return "admin/print-bill";
    }
